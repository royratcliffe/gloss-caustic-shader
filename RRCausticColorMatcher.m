...
#define BCOMP (2) // brightness component
	// Derive and blend two colours: non-caustic and caustic. The first, non-caustic, derives directly from the input triple (hsb argument). Each triple has three components, of course: hue, saturation and brightness, in that order.
	// If the saturation is too low, pretend the input colour exactly matches the caustic colour (yellow by default). Increase the saturation too. Therefore, all low-saturation colours snap to caustic yellow. Low saturation amounts to grey.
	...
	// Red-hue threshold marks the dividing line between blue and red. Every hue at the threshold or above shifts down and becomes a negative hue. It thereby escapes the blue-hue threshold and receives the default yellow caustic hue.
	...
	// At this point, the caustic fraction lies in the range -1 to 1. Caustic hues equate to 1/6 or 5/6 for yellow or magenta, respectively. Input comparison hue equates to (redHueThreshold-1) to redHueThreshold, low to high range. Where, since 0<=redHueThreshold<=1 holds true, the caustic fraction easily falls within the -1,1 interval. The fraction measures the different between the caustic hue and the input hue.
